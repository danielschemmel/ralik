...
pub(crate) fn is_alphabetic(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_alphabetic())?)
}

pub(crate) fn is_alphanumeric(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
...
pub(crate) fn is_ascii_alphabetic(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_alphabetic())?)
}

pub(crate) fn is_ascii_alphanumeric(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_alphanumeric())?)
}

pub(crate) fn is_ascii_control(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_control())?)
}

pub(crate) fn is_ascii_digit(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_digit())?)
}

pub(crate) fn is_ascii_graphic(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_graphic())?)
}

pub(crate) fn is_ascii_hexdigit(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_hexdigit())?)
}

pub(crate) fn is_ascii_lowercase(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_lowercase())?)
}

pub(crate) fn is_ascii_punctuation(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_punctuation())?)
}

pub(crate) fn is_ascii_uppercase(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_uppercase())?)
}

pub(crate) fn is_ascii_whitespace(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_ascii_whitespace())?)
}

pub(crate) fn is_control(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_control())?)
}

pub(crate) fn is_digit(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(2)?;
	let this = arguments.as_char(0)?;
...
pub(crate) fn is_lowercase(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_lowercase())?)
}

pub(crate) fn is_numeric(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_numeric())?)
}

pub(crate) fn is_uppercase(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_uppercase())?)
}

pub(crate) fn is_whitespace(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_bool(context, this.is_whitespace())?)
}

pub(crate) fn len_utf16(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
	Ok(Value::new_integer(context, this.len_utf16())?)
}

pub(crate) fn len_utf8(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_char(0)?;
