...
pub(crate) fn abs(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
...
/*
pub(crate) fn checked_div(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
...
pub(crate) fn is_negative(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_integer(0)?;
	Ok(Value::new_bool(context, this.is_negative())?)
}

pub(crate) fn is_positive(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_integer(0)?;
	Ok(Value::new_bool(context, this.is_positive())?)
}

pub(crate) fn pow(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(2)?;
	let this = arguments.as_integer(0)?;
...
pub(crate) fn signum(context: &Context, arguments: &[Value]) -> Result<Value, RuntimeError> {
	arguments.check_len(1)?;
	let this = arguments.as_integer(0)?;
