...
pub(crate) fn is_char_boundary(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(2)?;
	let this = arguments.as_string(0)?;
...
pub(crate) fn is_empty(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
...
pub(crate) fn repeat(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(2)?;
	let this = arguments.as_string(0)?;
	let arg = arguments.as_integer(1)?.to_usize().ok_or_else(|| Overflow::USize)?;
...
pub(crate) fn to_lowercase(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
...
pub(crate) fn to_uppercase(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
	Ok(Value::new_string(context, this.to_uppercase())?)
}

pub(crate) fn trim(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
	Ok(Value::new_string(context, this.trim())?)
}

pub(crate) fn trim_end(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
	Ok(Value::new_string(context, this.trim_end())?)
}

pub(crate) fn trim_start(context: &Context, arguments: &[Value]) -> Result<Value, CallError> {
	arguments.check_len(1)?;
	let this = arguments.as_string(0)?;
